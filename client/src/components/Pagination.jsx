  ...
  const btnList = btnArr.map((btn) => {
    return (
      <button
        key={btn}
        className={`${
          btn === page ? "border-2 border-blue-600" : ""
        } text-gray-900 bg-gray-200 px-4 py-2 rounded shadow text-sm font-medium`}
        ...
        <button
          ...
          className={`${
            totalPages > btnCount
              ...
              : "opacity-80"
          } bg-gray-200 text-gray-900 transition-all duration-400 rounded shadow font-medium py-2 px-4`}
        >
          Prev
        </button>
        {btnList}
